...
@HostListener('ionScroll',['$event']) onContentController($event: any){
  ...
  this.domCtrl.write(() => {
    ...
    this.renderer.setStyle(this.cover, 'padding-left', `${newOpacity}%`);
    this.renderer.setStyle(this.cover, 'padding-right' , `${newOpacity}%`);
  })
}
